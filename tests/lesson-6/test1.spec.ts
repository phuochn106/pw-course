...
test('Register successfully and check table data', async ({ page }) => {
    ...
    //xpathProductPage = "//a[contains(text(), 'Product Page')]";
    // this.cssTodoPage = "a[href='/todo']";
    // this.personalNote = page.locator('#personalNote');
    ...
    await test.step('Fill form and submit', async () => {
        await registerPage.fillUsername('phuoc2');
        await registerPage.fillEmail('phuoc2@yopmail.com');
        await registerPage.checkGender('female');
        await registerPage.clickRegister();
    });

    await test.step('Verify registered info in table', async () => {
        const isDisplayed = await registerPage.isUserInfoDisplayed('phuoc2', 'phuoc2@yopmail.com');
